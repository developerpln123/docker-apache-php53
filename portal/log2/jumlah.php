<?php
	...
	$q1 = "SELECT jumlah_pegawai,jumlah_non_pegawai FROM jml_pegawai WHERE bulan = '".date('m-Y', strtotime('-1 month'))."'";
	$res = mysql_query($q1);
	...
	$var_year = substr(date('m-Y', strtotime('-1 month')),3,4);
	$var_month = substr(date('m-Y', strtotime('-1 month')),0,2);
	if(substr(date('m-Y', strtotime('-1 month')),0,1) == 0) $var_month = substr(date('m-Y', strtotime('-1 month')),1,1);
	$var_month -= 1;
